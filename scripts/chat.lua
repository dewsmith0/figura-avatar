...
local CONFIG = {
    ...
    textColor = "#B34CFD",                           -- Text color (can be hex like "#FFFFFF" or MC color names like "gold")
    outlineColor = vectors.hexToRGB("#730CBD"),  -- RGB values for the outline (Must be hex)
...
local BugSpeak = require("libraries.BugSpeak")
local speech = BugSpeak.new()

speech:setVolume(0.5)
    :addSound('sounds.beep')
    :addSound('sounds.bop')
    :setBasePitch(2.5)
    :setPitchRange(0.8)
    :setSpacesPause(true)
